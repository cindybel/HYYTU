...
window.ItemPreviews=(()=>{
 ...
 function photograph(item){
  ...
  try{const box=new THREE.Box3().setFromObject(object),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3()),radius=Math.max(.1,size.length()/2);object.position.sub(center);
   const camera=new THREE.PerspectiveCamera(32,4/3,.01,100);camera.position.set(radius*.85,radius*.62,radius*(item.category==='clothing'?3.2:3.65));camera.lookAt(0,0,0);renderer.render(scene,camera);return renderer.domElement.toDataURL('image/png');
