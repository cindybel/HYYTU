...
window.StudioStorage=(()=>{
 ...
 function rebuild(list){
  dispose();const limit=capacity(window.StudioWorld?.visitingHome?.label||profile.housing),bays=Math.max(1,Math.min(4,Math.max(Math.ceil(limit/80),Math.ceil(list.length/16))));
