...
window.StudioSet=(()=>{
 ...
 windowGlass.emissive=new THREE.Color(0x142332);windowGlass.emissiveIntensity=.35;
 let homeSignature='';
 function updateHome(){
  const title=profile?.housing?.type||'';if(title===homeSignature)return;homeSignature=title;
  for(const child of [...homeDecor.children]){homeDecor.remove(child);child.geometry?.dispose();}
  ...
  if(garage)return;
  box(right-left,height,.16,0,height/2,front-.04,industrial?brick:plaster,homeDecor);
  const basement=/sous-sol/.test(title),premium=/premium|luxe|penthouse/.test(title),soundproof=/insonorise/.test(title);
  ...
  for(let i=0;i<count;i++){
   ...
   box(width+.14,wh+.14,.10,x,wy,front-.17,windowFrame,homeDecor);
  ...
  if(soundproof||/partage/.test(title))for(const x of [-5.7,-4.7,4.7,5.7]){
 ...
 return {group,tick(walking){scanLegacy();if(walking)updateHome();group.visible=walking;legacy.forEach(o=>{if(o.userData.__starterGarageHidden)o.visible=false;});},blocked};
})();
