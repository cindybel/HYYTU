...
window.StudioStorage=(()=>{
 ...
 const bayLayouts=[
  ...
  {x:5.35,z:9.35,r:-Math.PI/2,label:'SIGNAL'},
  {x:-5.35,z:6.55,r:Math.PI/2,label:'ARCHIVES'},
 ];
 ...
 function rebuild(list){
  dispose();const limit=capacity(),bays=Math.max(1,Math.min(4,Math.max(Math.ceil(limit/80),Math.ceil(list.length/16))));
  ...
  function shelfBay(layout,bay){
   ...
   for(const yy of [.28,1.0,1.72,2.44,3.16])box(2.15,.07,.62,0,yy,0,wood);
   for(const dx of [-1.02,1.02])box(.065,3.22,.065,dx,1.63,.22,steel);
   box(2.15,3.22,.035,0,1.63,-.33,dark);
   const sign=labelMesh(bay===0?`${layout.label} · ${used()} / ${limit}`:layout.label,1.9);sign.position.set(0,3.43,.34);holder.add(sign);
   return holder;
  }
 ...
 function near(state){return group.visible&&group.children.length&&bayLayouts.some(layout=>{const p=localPoint(layout,state);return Math.abs(p.x)<1.55&&Math.abs(p.z)<1.55;});}
 function open(){if(!group.visible)return;StudioWorld.showComputer();document.body.classList.remove('studio-world-view');openApp('inventory');PhysicalPanels.open('inventory');}
 function tick(dt,walking){group.visible=walking;if(!walking||!profile?.created)return;elapsed+=dt;if(elapsed<.5)return;elapsed=0;const list=displayEntries(),key=JSON.stringify([profile.housing.type,profile.gear?.computer,profile.gear?.projector,list.map(e=>[e.item.id,e.count])]);if(key!==signature){signature=key;rebuild(list);}}
 ...
 function blocked(x,z){return bayLayouts.some(layout=>{const p=localPoint(layout,{x,z});return Math.abs(p.x)<1.18&&Math.abs(p.z)<.52;});}
