...
function createShopItems() {
  ...
  [
    ...
    ['standard', 'Epsun EB-Lo 4200 3LCD', 420, 'Image stable et couleurs propres, proche des gammes installation 3LCD.', { technique: 4 }],
...
function createCareer() {
  ...
  saveSlots();
  applyAppearanceToPlayer();
  notify('Carriere creee. Tu commences dans le garage avec 75$.');
  closeAppWindow(false);
  clearInactiveWindows();
  showScreen('desktop');
}
...
function updateSceneContextVisibility() {
  ...
  if (stageGroup) stageGroup.visible = !choosingLook;
...
function activateGodMode() {
  ...
  profile.skills = Object.fromEntries(skillCatalog.map((skill) => [skill.id, 3]));
  ...
  shopItems.forEach((item) => {
    if (item.stackable || ['cable', 'adapter'].includes(item.type)) profile.inventory[item.id] = 6;
  });
      ...
      <section class="email-list">
        ${profile.emails.length === 0 ? '<div class="email-card"><p>Aucun email pour l instant.</p></div>' : profile.emails.slice().reverse().map((email) => `
          <button class="email-row${email.id === selectedEmailId ? ' selected' : ''}${email.read ? '' : ' unread'}" type="button" data-email="${email.id}">
            <span>${email.read ? '' : 'â—'} ${escapeHtml(email.from)}</span>
...
function renderGigCard(gig, featured = false, levelGap = 0) {
  ...
  return `
    <article class="social-post gig-card ${statusClass}${featured ? ' featured-gig' : ''} style-${escapeHtml(gig.style)}">
      ...
      <p>${escapeHtml(gig.requirement)}</p>
...
function renderFinance() {
  ...
  const netMonthly = monthIncome - monthExpenses - projectedMonthCost;
  const projectedBalance = Math.round(profile.money + netMonthly);
  appWindow.innerHTML = `
    <div class="app-heading">
      <div>
        <h1>Finance</h1>
    ...
    <div class="finance-grid">
      ...
      <section class="finance-card">
        <span>Depenses</span>
        <strong>${monthExpenses + monthSpent + minPayments}$</strong>
    ...
    <div class="dashboard-grid finance-details">
      ...
      <section class="card">
        ...
        <div class="finance-row"><span>Projection mois</span><strong>${netMonthly >= 0 ? '+' : ''}${netMonthly}$</strong><em>revenus - depenses - dettes</em></div>
        <div class="finance-row"><span>Solde projete</span><strong>${projectedBalance}$</strong><em>si tout passe ce mois</em></div>
      </section>
      <section class="card">
        ...
        <div class="finance-row total-row"><span>Total mensuel</span><strong>${netMonthly >= 0 ? '+' : ''}${netMonthly}$</strong><em>projection claire</em></div>
      </section>
      <section class="card">
...
function recordFinance(amount, label) {
  if (!profile.financeLog) profile.financeLog = [];
  profile.financeLog.push({ day: profile.day, amount: Math.round(amount), label });
  ...
  appWindow.innerHTML = `
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        ...
        <p>Confort ${profile.housing.comfort}/30. Le loyer est traite le 1er du mois.</p>
      </div>
    </div>
  `;
}
...
function renderHousing() {
  ...
  appWindow.innerHTML = `
    <div class="app-heading">
      <div>
        <h1>Immobilier</h1>
        <p>Choisis ton espace de vie et de creation. Le loyer est paye le 1er du mois.</p>
      </div>
      <div class="shop-wallet">
        <span>Budget</span>
    ...
    <div class="housing-current">
      <div><span>Adresse actuelle</span><strong>${escapeHtml(profile.housing.type)}</strong></div>
      <div><span>Loyer</span><strong>${profile.housing.rent}$ / mois</strong></div>
      <div><span>Rangement</span><strong>${window.StudioStorage?.capacity() ?? 24} places</strong></div><div><span>Confort</span><strong>${profile.housing.comfort}/44</strong></div>
    </div>
    <section class="shop-section">
      <div class="shop-section-head">
        <div>
          <h2>Appartements et studios</h2>
          <p>Plus l'espace est bon, plus ta fatigue baisse et ton image devient credible.</p>
        </div>
        <span>${items.length} lieux</span>
      </div>
      <div class="shop-grid">
...
function rewardSkillPracticeFromGig(gig, score, minScore) {
  if(gig?.sessionReady&&window.SessionRules)return SessionRules.reward(gig);
    ...
    appWindow.innerHTML = `
      ...
      <section class="shop-marketing-hero">
        <div class="shop-marketing-copy">
          ...
          <p>AchÃ¨te du gear, ameliore ton espace de creation et developpe un look reconnaissable. Chaque item change tes stats avec un avantage et un compromis.</p>
          <div class="shop-marketing-actions">
            ...
            <button class="secondary-action" type="button" data-shop-view="clothing">Cabine vÃªtements</button>
          </div>
        </div>
      ...
      <div class="shop-market-stats">
        ...
        <span><strong>${shopItems.filter((item) => (item.category || 'gear') === 'clothing').length}</strong> vÃªtements</span>
      </div>
  ...
  appWindow.innerHTML = `
    ...
    <section class="product-detail-page">
      ...
      <div class="product-detail-info">
        ...
        <p>${escapeHtml(item.description)}</p><small class="effect-scope">${escapeHtml(item.effectScope||'')}</small>
...
function beginGigFromSetup() {
  ...
  recordFinance(-loadout.totalCost, `Uver${loadout.rentalCost ? ' + gear' : ''} ${gig.title}`);
...
function getGigMinimumScore(gig) {
  return Number(gig?.minScore) || 100;
}

function getGigMaskRequirement(gig) {
  return Number(gig?.maskRequired) || 0;
}
...
function createRig(index, screenX, screenWidth, screenHeight) {
  ...
  addScreenFrame(group, screen);
  addScreenGuides(group, screen);
...
function updateProjectionMeshes(rig) {
  ...
  rig.screenProjection.material.opacity = ready ? 0.96 : 0.62 + rig.calibration.coverage / 300;
...
function updateCable(rig) {
  ...
  const c = rig.calibration;
  const end = new THREE.Vector3(c.projectorX + Math.sin(c.yaw) * 0.22, 0.82, c.projectorZ + 0.58);
...
function getObjectiveLine(connected, minScore, maskRequired, perfect) {
  ...
  if (perfect) {
    return 'Setup pret. Clique Terminer pour valider le contrat et recevoir tes points.';
  }
...
function togglePenMode() {
  ...
  message.textContent = penMode
    ? 'Plume active: clique sur un ecran pour poser des points de contour.'
...
function addMaskPointFromHit(hit) {
  ...
  if (rig.maskPoints.length > 8) rig.maskPoints.shift();
  rig.maskQuality = Math.min(100, rig.maskPoints.length * 25);
...
function finishGig() {
  ...
  profile.money += money;
  recordFinance(money, `Paiement ${currentGig.title}`);
  ...
  if(currentGig.sessionReady){profile.showSessions=Number(profile.showSessions||0)+1;currentGig.lastSessionNumber=profile.showSessions;}
  ...
  resultContent.innerHTML = `
    <div class="result-list">
      <div class="contract-debrief">
        ...
        <div><small>CE QUI A FONCTIONNÉ</small><p>${liveResult.seconds > 0 && liveResult.score >= 80 ? 'Tu as bien accompagné l’énergie musicale.' : connectedRatio >= 1 ? 'Tes projecteurs reçoivent le signal.' : 'Tu as repéré les étapes de préparation du contrat.'} ${liveResult.transitions > 0 ? 'Tu as réalisé un changement en fondu.' : ''}</p></div>
        ...
        ${gainedSkills.length ? `<p class="contract-practiced">Compétence acquise : ${escapeHtml(gainedSkills.join(', '))}.</p>` : ''}
...
function calculateCareerScore(baseSkill, gig, loadout = null, run = {}) {
  if(gig?.sessionReady&&window.SessionRules)return SessionRules.score(baseSkill,gig,run);
...
function onCanvasPointerUp(event) {
  ...
  if (!cameraControl.moved && event.button === 2) selectFromPointer(event);
}
...
function selectFromPointer(event) {
  ...
  if (penMode) {
    const screenHit = raycaster.intersectObjects(rigs.map((rig) => rig.screenProjection), true)[0] ||
