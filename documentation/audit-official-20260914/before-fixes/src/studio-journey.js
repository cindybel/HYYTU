...
window.StudioLife=(()=>{
 ...
 function box(w,h,d,x,y,z,m,parent=props){const mesh=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),m);mesh.position.set(x,y,z);mesh.castShadow=true;mesh.receiveShadow=true;parent.add(mesh);return mesh;}
 ...
 // Single fold-out bed along the left wall.
 const bedX=-5.05,bedZ=9.55;
 box(2.0,.32,3.85,bedX,.31,bedZ,mat(0x28323a));
 box(1.84,.22,3.68,bedX,.58,bedZ,mat(0x68737b));
 box(1.52,.16,.62,bedX,.8,bedZ-1.42,mat(0xb9b09d));
 label('DORMIR',-6.52,1.55,bedZ,Math.PI/2,1.45);
 ...
 const projectorX=4.05,projectorZ=10.15;
 ...
 function rebuildEquippedGear(){
  ...
  if(projectorData){
   const model=fitModel(projectorData.model,.86);model.position.set(projectorX,1.12,projectorZ);model.rotation.y=-Math.PI/2;equipment.add(model);
   label(projectorData.item.label,projectorX,1.82,projectorZ-.52,Math.PI,1.9,equipment);
  }
 }
 ...
 function sleepHours(hours){
  ...
  for(let i=0;i<crossed;i++){
   profile.calendarLog ||= {};
   if(!profile.calendarLog[profile.day])profile.calendarLog[profile.day]={label:`Sommeil ${amount} h`,detail:'Repos au garage',activities:['sleep']};
   if(typeof advanceDay==='function')advanceDay();
  }
  if(profile?.stats)profile.stats.fatigue=Math.max(0,(Number(profile.stats.fatigue)||0)-amount*7);
 ...
 function act(s){
  ...
  if(z==='mixer'){
   ...
   launch.click();const free=document.querySelector('#academy [data-free]');if(free)free.click();return;
  }
  if(z==='projector'){
 ...
 function blocked(x,z){
  if(x<bedX+1.05&&x>bedX-1.05&&z>bedZ-2.0&&z<bedZ+2.0)return true;
 ...
 return {hint,act,blocked,tick,openSleepMenu,sleepHours};
})();
