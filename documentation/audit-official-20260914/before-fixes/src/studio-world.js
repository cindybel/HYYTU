...
window.StudioWorld=(()=>{
 ...
 function advanceWorld(seconds){init();const amount=Math.max(0,Number(seconds)||0);state.seconds=Math.max(0,state.seconds+amount);return state.seconds;}
 ...
 function tick(dt){
  ...
  stepWorldClock(dt,!document.hidden&&(desktop||gig)&&!liveShow?.paused);
  ...
  const clock=clockParts();writeText(clockNode,`${String(clock.hour).padStart(2,'0')}:${String(clock.minute).padStart(2,'0')}`,'clock');
  ...
  const action=getWorldAction(),disabled=!action?.enabled,actionText=action?.label||'Approche-toi d’un objet interactif';if(lastButtonDisabled!==disabled){lastButtonDisabled=disabled;actionButton.disabled=disabled;}writeText(actionButton,actionText,'action');writeText(hintNode,action?.hint||(near()?'Ordinateur : booking, messages, boutique et carrière.':'Explore le garage : les objets importants sont utilisables directement.'),'hint');
 }
 ...
 return {enter,tick,showComputer(){computer=true;held.clear();},advanceMinutes(minutes){return advanceWorld(Math.max(0,Number(minutes)||0)*60);},getClock:clockParts,get computerOpen(){return computer;}};
})();
