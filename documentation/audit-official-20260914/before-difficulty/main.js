...
function renderHousing() {
  ...
  appWindow.innerHTML = `<div class="app-heading"><div><h1>Locaux</h1><p>Un espace pour vivre, créer et ranger ton matériel.</p></div><div class="shop-wallet"><span>Disponible</span><strong>${profile.godMode ? '∞' : profile.money + '$'}</strong></div></div>
    ...
    <div class="shop-grid">${items.map(item => {
      ...
      return `<article class="shop-card"><div class="shop-card-body"><h2>${escapeHtml(item.label)}</h2><p>${escapeHtml(item.description)}</p><p><strong>${item.rent}$ / mois</strong> · ${window.StudioStorage?.capacity(item.label) ?? 24} places</p><p>Installation : ${cost}$${owned ? ' · déjà débloqué' : ''}</p>${issue ? `<p>${escapeHtml(issue)}</p>` : ''}</div><div class="row-actions"><button data-home-preview="${item.id}">Maquette 3D</button><button class="primary-action" data-home-move="${item.id}" ${current || locked || issue || insufficient ? 'disabled' : ''}>${current ? 'Lieu actuel' : locked ? 'Niveau ' + level + ' requis' : insufficient ? 'Budget insuffisant' : 'Choisir ce lieu'}</button></div></article>`;
    }).join('')}</div>`;
  appWindow.querySelectorAll('[data-home-preview]').forEach(button => button.onclick = () => ItemInspector.open(button.dataset.homePreview));
