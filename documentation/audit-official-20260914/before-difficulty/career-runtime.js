...
function captureActiveRun() {
  ...
  profile.activeRun = {
    ...
    rigs:rigs.map(rig=>({calibration:{...rig.calibration}, connected:rig.cable.connected, maskPoints:rig.maskPoints.map(p=>[p.x,p.y]),maskQuality:rig.maskQuality})),
  };
}
function resumeActiveRun() {
  ...
  saved.rigs?.forEach((state,index)=>{
    ...
    rig.maskPoints=(state.maskPoints||[]).map(([x,y])=>new THREE.Vector2(x,y));rig.maskQuality=Number(state.maskQuality)||0;updateMaskContour(rig);
  });
  ...
  if(saved.live){startLivePerformance();liveShow.restore(saved.live);}
