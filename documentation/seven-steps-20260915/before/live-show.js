...
class VJLiveShow {
  constructor(root, clips) {
    ...
    root.innerHTML = `
      ...
      <div class="live-response" data-live-response role="status">Écoute le groove, puis règle ton intensité.</div><details class="live-help"><summary>Pourquoi ?</summary><p>La bande-son suit quatre passages écrits pour apprendre : warm-up, montée, peak et break. Elle ne détecte pas une musique importée. Le retour évalue ton intensité et tes fondus, jamais la beauté de tes choix.</p><label><input type="checkbox" data-live-music> Bande-son du show</label></details><div class="live-desk">
        ...
        <div class="live-mix-controls"><label>Durée du fondu <select data-live-duration><option value="0.6">Rapide · 0,6 s</option><option value="2" selected>Doux · 2 s</option><option value="4">Lent · 4 s</option></select></label><button type="button" data-live-take>Envoyer en fondu →</button><p data-live-status aria-live="polite">Prépare un clip sans modifier la sortie.</p></div>
        <div class="live-level-controls"><label>Intensité de sortie <output data-live-level>50 %</output><input data-live-intensity type="range" min="0" max="100" value="50" aria-label="Intensité de sortie" /></label><button type="button" data-live-blackout aria-pressed="false">Blackout</button><small>Coupe l’image en salle. Le clip continue de tourner.</small></div>
    ...
    ClipBrowser.mount(deck.querySelector('.resolute-live-clips'),clips,index=>{if(this.paused||this.transition||this.completed)return false;this.prepare(index);},this.previewIndex);
    ...
    this.select.addEventListener('change',()=>this.prepare(Number(this.select.value)));
  ...
  prepare(index) {
    if(this.paused || this.transition || this.completed) return;
  ...
  take() {
    ...
    this.transition={elapsed:0,duration:Number(this.root.querySelector('[data-live-duration]').value)};
    this.select.disabled=true;this.root.querySelector('[data-live-take]').disabled=true;
    this.status('Fondu en cours : les deux clips sont mélangés dans la projection.');
  }
  status(text) { this.root.querySelector('[data-live-status]').textContent=text;this.root.querySelectorAll('[data-resolute-clip]').forEach(button=>{button.setAttribute('aria-pressed',String(Number(button.dataset.resoluteClip)===this.previewIndex));button.disabled=Boolean(this.paused||this.transition||this.completed);}); }
  updateCue() {
  ...
  setPaused(value) {
    ...
    this.select.disabled=value||Boolean(this.transition);this.root.querySelector('[data-live-take]').disabled=value||Boolean(this.transition);
  ...
  drawSource(video,alpha){
    ...
    if(this.effects.mirror){this.ctx.translate(640,0);this.ctx.scale(-1,1);}
    this.ctx.drawImage(video,sx,sy,cropW,cropH,0,0,640,360);
    this.ctx.restore();
  }
  drawProgram(fraction=0){
    this.ctx.save();this.ctx.globalCompositeOperation='source-over';this.ctx.globalAlpha=1;this.ctx.filter='none';this.ctx.fillStyle='#000';this.ctx.fillRect(0,0,640,360);this.ctx.restore();
  ...
  tick(delta) {
    ...
    const fraction=this.transition?Math.min(1,(this.transition.elapsed+=step)/this.transition.duration):0;
  ...
  result() {
    return {score:Math.round(this.elapsed?this.integral/this.elapsed:0),completion:Math.min(1,this.elapsed/this.duration),transitions:this.transitions,effectChanges:this.effectChanges,seconds:Math.round(this.elapsed),phases:this.phaseStats.map((p,i)=>({name:this.cues[i].name,seconds:p.seconds,matched:p.matched}))};
  }
  snapshot() {
    return {...this.result(),paused:this.paused,elapsed:this.elapsed,integral:this.integral,intensity:this.intensity,blackout:this.blackout,effects:{...this.effects},currentIndex:this.currentIndex,previewIndex:this.previewIndex,transition:this.transition?{...this.transition}:null,programTime:this.program?.currentTime||0,previewTime:this.preview?.currentTime||0};
  }
  restore(saved) {
    ...
    this.select.disabled=Boolean(this.transition)||this.completed;this.root.querySelector('[data-live-take]').disabled=Boolean(this.transition)||this.completed;
