...
window.StudioLayouts=(()=>{
 ...
 const steel=mat(0x263338,.38,.65),oak=mat(0x887051),paper=mat(0xe1ddcf),ink=mat(0x354950),stone=mat(0xc0b9a9,.48),book=mat(0x94735d);
 ...
 let bounds=[];
 function box(parent,w,h,d,x,y,z,m){const mesh=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),m);mesh.position.set(x,y,z);mesh.castShadow=true;mesh.receiveShadow=true;parent.add(mesh);return mesh;}
 function cushion(parent,w,h,d,x,y,z,m){
  const r=.035,shape=new THREE.Shape();shape.moveTo(-w/2+r,-h/2);shape.lineTo(w/2-r,-h/2);shape.quadraticCurveTo(w/2,-h/2,w/2,-h/2+r);shape.lineTo(w/2,h/2-r);shape.quadraticCurveTo(w/2,h/2,w/2-r,h/2);shape.lineTo(-w/2+r,h/2);shape.quadraticCurveTo(-w/2,h/2,-w/2,h/2-r);shape.lineTo(-w/2,-h/2+r);shape.quadraticCurveTo(-w/2,-h/2,-w/2+r,-h/2);
  const g=new THREE.ExtrudeGeometry(shape,{depth:d-.04,bevelEnabled:true,bevelThickness:.02,bevelSize:.02,bevelSegments:2,steps:1,curveSegments:4});g.translate(0,0,-(d-.04)/2);
 ...
 function seat(parent,width,cloth){
  ...
  cushion(parent,width-.22,.18,.78,0,.59,-.03,cloth);cushion(parent,width,.57,.18,0,.85,.38,cloth);
 ...
 function build(parent,id){
  ...
  for(const [kind,x,z] of config.parts){
   ...
   if(kind==='kitchen'){
    ...
    box(p,.5,.015,.37,-.4,.968,0,steel);box(p,.05,.3,.05,-.4,1.12,.18,steel);box(p,.05,.04,.18,-.4,1.25,.11,steel);
   }
