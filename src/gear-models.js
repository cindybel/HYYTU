...
window.GearModels=(()=>{
 ...
 function create(item){
  ...
  function ports(n,x,y,z,spacing=.105,back=false){if(window.PhysicalV4?.enabled)return;for(let i=0;i<n;i++){box(.078,.04,.018,x+i*spacing,y,z,metal,.004);box(.058,.021,.02,x+i*spacing,y,z+(back?-.009:.009),rubber,.002);}}
  ...
  function display(w,h,x,y,z){
   ...
   if(window.PhysicalV4?.enabled){d.fillStyle='#18212b';d.fillRect(0,0,768,432);d.fillStyle='#dabc80';d.font='bold 26px Arial';d.fillText('POSTE VIDEO / DIAGNOSTIC',32,52);d.fillStyle='#b7d3d7';d.font='21px Arial';['1. Alimenter les appareils','2. Relier les ports OUT → IN','3. Choisir le bon input','4. Tester et cadrer'].forEach((s,i)=>d.fillText(s,32,120+i*64));}d.fillStyle='#475767';d.fillRect(18,404,732,8);const t=new THREE.CanvasTexture(c);t.encoding=THREE.sRGBEncoding;return add(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({map:t,toneMapped:false}),x,y,z);
  }
