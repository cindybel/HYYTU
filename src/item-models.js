...
window.ItemModels={create(item){
 ...
 if(item.category==='gear'&&window.GearModels)return GearModels.create(item);
 if(item.category==='clothing'&&window.ClothingModels)return ClothingModels.create(item);
 ...
 if(item.category==='vjloop'){
  ...
  const palette={'hoodie-vj':0x272b34,'jacket-blackout':0x252b36,'jacket-reflective':0xb9c8d0,'coat-tour':0x514954,'vest-led':0x293445,'full-fit-pro':0x384561,'pants-cargo':0x303c37,'pants-tech':0x293448,'shoes-cyan':0x36d6da,'shoes-magenta':0xed58b2,'cap-low':0x28313e,'beanie-black':0x24222d,'gloves-stage':0x242936,'mask-club':0x303649};
  cloth.color.setHex(palette[id]??0x272b34);if(id==='jacket-reflective'){cloth.roughness=.38;cloth.metalness=.22;}if(id==='shoes-magenta')glass.color.setHex(0xed58b2);
  ...
  if(slot==='headphones'){ring(.34,.045,0,.55,0,metal);for(const x of [-.34,.34]){const ear=cyl(id.endsWith('pro')?.17:.13,.13,x,.45,0,black);ear.rotation.z=Math.PI/2;}}
   ...
   const long=id==='coat-tour',vest=id==='vest-led',hood=id==='hoodie-vj';box(.65,long?1.05:.68,.2,0,long?.6:.8,0,cloth);for(const side of [-1,1]){if(!vest)box(.22,long?.83:.65,.2,side*.45,.74,0,cloth).rotation.z=side*.2;box(.21,.16,.04,side*.18,.67,.13,black);}rod([-.4,1.3,0],[0,1.5,0]);rod([0,1.5,0],[.4,1.3,0]);ring(.07,.012,0,1.56,0,metal);if(hood){const logo=ring(.07,.012,0,.89,.116,glass);logo.scale.set(1,.9,1);const o=new THREE.Mesh(new THREE.SphereGeometry(.25,20,12),cloth);o.scale.set(1,1,.6);o.position.set(0,1.22,-.06);group.add(o);}if(['jacket-reflective','vest-led','full-fit-pro'].includes(id)){for(const x of [-.22,.22])box(.045,.48,.025,x,.88,.12,id==='vest-led'?glass:metal);}box(.018,.67,.015,0,.8,.115,metal);if(id==='full-fit-pro')for(const x of [-.18,.18])box(.26,.45,.18,x,.08,0,cloth);
  }
