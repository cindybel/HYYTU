...
window.StudioLife=(()=>{
 ...
 function createProjectorCart(){const cart=new THREE.Group();cart.name='Studio projector cart';
  box(.78,.08,.78,0,.04,0,mat(0x424a4e,.55,.4),cart);
  box(.09,.96,.09,0,.52,0,mat(0x5a646a,.45,.55),cart);
  box(.9,.08,.8,0,1,0,mat(0x424a4e,.55,.4),cart).userData.placementSurface='Rack du projecteur';
  box(.58,.05,.58,0,.10,0,mat(0x313a40,.55,.35),cart);return cart;}
 const legacyCart=createProjectorCart();legacyCart.position.set(projectorX,.08,projectorZ);props.add(legacyCart);
 ...
 function hint(s){const z=zone(s);if(window.PhysicalV4?.enabled&&z!=='bed')return null;if(z==='bed')return {enabled:true,label:'Dormir · choisir la durée · E',hint:'Tu peux dormir quand tu veux, de 1 à 12 heures.'};if(z==='mixer')return {enabled:true,label:'Ouvrir le poste VJ · E',hint:'Ordinateur équipé + contrôleur VJ. Lance la pratique libre.'};if(z==='projector')return {enabled:true,label:'Tester mon projecteur · E',hint:'Le projecteur visible correspond à celui équipé.'};return null;}
 ...
 function blocked(x,z){
  ...
  if(!window.PhysicalV4?.enabled&&x>projectorX-.62&&x<projectorX+.62&&z>projectorZ-.62&&z<projectorZ+.62)return true;
...
window.StudioJourney=(()=>{
 ...
 function act(s){
  if(window.PhysicalV4?.enabled&&profile.physical?.prepared){PhysicalV4.depart();return;}
