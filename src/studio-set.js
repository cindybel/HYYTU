...
window.StudioSet=(()=>{
 ...
 function makeBrickTexture(){
  ...
  q.fillStyle='#696560';q.fillRect(0,0,cv.width,cv.height);
  const bw=126,bh=58,mortar=5;
  for(let row=0,y=0;y<cv.height;row++,y+=bh){
   ...
   for(let x=offset;x<cv.width+bw;x+=bw){
    ...
    const r=104+seed*1.5,g=88+seed*1.25,b=75+seed;
 ...
 const brick=new THREE.MeshStandardMaterial({color:0xc9c3b9,map:brickTexture,roughness:.96,metalness:0});
 ...
 // Shell.
 box(right-left,.16,front-back,0,.08,(back+front)/2,floorMat).userData.placementSurface='Sol';
 ...
 box(2.9,.14,.72,benchX,.87,benchZ,wood).userData.placementSurface='Établi';
 ...
 for(let i=0;i<4;i++){const hook=new THREE.Mesh(new THREE.TorusGeometry(.11,.022,8,16,Math.PI*1.55),rubber);hook.rotation.x=Math.PI/2;hook.position.set(benchX-.75+i*.5,1.04,back+.29);hook.userData.legacyCable=true;group.add(hook);}
 ...
 const cable=new THREE.Mesh(new THREE.TorusGeometry(.27,.03,8,24),rubber);cable.rotation.x=Math.PI/2;cable.position.set(benchX-.55,.98,benchZ);cable.userData.legacyCable=true;group.add(cable);
 ...
 for(let row=0;row<8;row++){
  oq.fillStyle=['#988776','#a08e79','#948371','#a69581'][row%4];oq.fillRect(0,row*64,512,64);
  for(let i=0;i<85;i++){oq.strokeStyle=i%3?'rgba(54,35,20,.035)':'rgba(233,207,165,.06)';oq.beginPath();const y=row*64+(i*17%62);oq.moveTo(0,y);oq.bezierCurveTo(160,y-Math.sin(i)*4,340,y+Math.sin(i*.4)*5,512,y);oq.stroke();}
  oq.fillStyle='#776957';oq.fillRect(0,row*64,512,.6);oq.fillRect((row*179)%512,row*64,.8,64);
 }
 ...
 function daylight(hour){
  const night=hour<6||hour>=20,dusk=hour>=17&&!night;
  const colors=night?['#10172c','#29354b','#63596a']:dusk?['#687e94','#c1aea3','#e1c4a2']:['#47789f','#b6ccd8','#e5dad0'];
  const gradient=skyContext.createLinearGradient(0,0,0,512);colors.forEach((c,i)=>gradient.addColorStop(i/2,c));skyContext.fillStyle=gradient;skyContext.fillRect(0,0,16,512);skyTexture.needsUpdate=true;
  cityWindow.color.set(night?0xe5bf7d:0xa8c4cf);warm.intensity=night?1.12:dusk?1:.80;cool.intensity=night?.36:.78;doorLight.intensity=night?.18:.65;
 }
 return {group,daylight,refreshHome(){homeSignature='';updateHome();},tick(walking){if(venueFloor)venueFloor.visible=!walking;scanLegacy();if(walking)updateHome();group.visible=walking;for(const o of group.children)if(o.userData.legacyCable)o.visible=!window.PhysicalV4?.enabled;legacy.forEach(o=>{if(o.userData.__starterGarageHidden)o.visible=false;});},blocked};
})();
