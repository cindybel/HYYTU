...
window.PhysicalStorage=(()=>{
 ...
 const DEFAULT_LAYOUT_VERSION=4;
 ...
 function applyDefaultProjectionCorner(state){
  ...
  // Match the approved reference more closely: the screen sits almost against the left garage door,
  // while the projector rack is farther right, with a clear walkway between both pieces.
  if(screen){screen.location='desk';screen.position={x:-5.08,y:.168,z:9.05};screen.rotation=Math.PI/2;}
  if(stand){stand.location='desk';stand.position={x:1.55,y:.168,z:8.55};stand.rotation=0;state.studioCartPlaced=true;}
  if(projector&&stand){projector.location='desk';projector.position={x:1.55,y:1.216,z:8.55};projector.rotation=-Math.PI/2;projector.mountedTo=stand.uid;projector.mountedOffset={x:0,y:1.048,z:0,rotation:-Math.PI/2};}
