...
window.PhysicalStorage=(()=>{
 ...
 const DEFAULT_LAYOUT_VERSION=3;
 ...
 function applyDefaultProjectionCorner(state){
  ...
  // Approved GOD99 reference: screen close to the left garage door, with the rack/projector
  // just to its right and slightly deeper into the room. This is intentionally a compact
  // projection corner rather than placing both pieces on opposite sides of the garage.
  if(screen){screen.location='desk';screen.position={x:-4.72,y:.168,z:7.72};screen.rotation=Math.PI/2;}
  if(stand){stand.location='desk';stand.position={x:-1.58,y:.168,z:7.35};stand.rotation=0;state.studioCartPlaced=true;}
  if(projector&&stand){projector.location='desk';projector.position={x:-1.58,y:1.216,z:7.35};projector.rotation=-Math.PI/2;projector.mountedTo=stand.uid;projector.mountedOffset={x:0,y:1.048,z:0,rotation:-Math.PI/2};}
