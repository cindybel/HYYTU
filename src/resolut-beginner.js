...
(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .resolut-beginner .resolut-exit{position:fixed;top:18px;right:28px;z-index:20;background:#203743;color:#f1f6f4;border:1px solid #597580;border-radius:6px;padding:10px 15px;font-size:13px;min-height:38px}
      .resolut-beginner .resolute-titlebar{padding-right:170px!important}
      .resolut-beginner .resolut-program canvas{height:auto!important;min-height:0!important;max-height:45vh!important;aspect-ratio:16/9;object-fit:contain!important}
      .resolut-beginner .resolut-program-stage{align-content:center}
      @media(min-width:1151px){
       .resolut-beginner .resolut-pro-workspace{min-height:420px!important}
       .resolut-beginner .academy-clips canvas{height:66px!important;max-height:66px!important;object-fit:contain}
       .resolut-beginner .resolut-preview-slot canvas{max-height:110px!important;object-fit:contain}
       .resolut-beginner .resolut-browser{grid-template-rows:auto auto auto}
      }
  ...
  function simplify() {
    ...
    const exit=academy.querySelector('header [data-close]');if(exit){exit.classList.add('resolut-exit');titlebar?.append(exit);}
    const title = titlebar?.querySelector('strong');
    if (title) title.innerHTML = '<i>A</i> Resolute 8 <small style="font-size:9px;color:#68eaf4;font-weight:700">MODE DÉBUTANT</small>';
    ...
    if (layer) layer.innerHTML = '<small>COUCHE 1</small><strong>VISUELS</strong><span>Prépare une image, puis envoie-la en fondu.</span>';
    ...
    head.innerHTML = '<small>3 · MIXER SIMPLE</small><strong>Ton mix</strong><p>Prépare le fondu, puis adapte l’intensité à la musique.</p>';
    params.prepend(head);
