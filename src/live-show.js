...
class VJLiveShow {
  constructor(root, clips, options={}) {
    ...
    this.genre=ClipProgression.genre((options.gig||currentGig)?.style);
    clips=ClipProgression.playlist(clips,options.gig||currentGig,options.saved);
    this.clips = clips;
    this.duration = 60;
    ...
    const task=document.createElement('section');task.className='live-action-card';task.setAttribute('aria-label','Ce que tu dois faire');task.innerHTML='<small>TON ACTION MAINTENANT</small><strong data-live-task-title></strong><p data-live-task-how></p><div class="live-gesture-progress"><progress data-live-gesture max="1" value="0" aria-label="Avancement du geste"></progress><span data-live-gesture-label></span></div><span data-live-task-result role="status"></span>';root.querySelector('.live-cue').before(task);
    ...
    root.querySelector('.live-action-card').before(guide.querySelector('.live-phase-track'));
    const meter=document.createElement('div');meter.className='live-target-meter';meter.setAttribute('aria-hidden','true');meter.innerHTML='<span data-meter-band></span><i data-meter-needle></i>';root.querySelector('[data-live-target]').after(meter);
    ...
    const suggestion=document.createElement('div');suggestion.className='live-visual-suggestion';suggestion.innerHTML='<span data-live-visual-hint></span><button type="button" data-live-suggest>Voir les clips conseillés</button>';deck.before(suggestion);
    root.querySelector('[data-live-suggest]').onclick=()=>{const filter=root.querySelector('[data-clip-energy]');filter.value=this.suggestedEnergy();filter.dispatchEvent(new Event('change'));filter.focus();};
    ...
    root.querySelector('.live-show-head small').textContent=`${ClipProgression.names[this.genre]} · ${this.showProfile.bpm} BPM · ${this.canvas.width} × ${this.canvas.height}`;
    const collection=document.createElement('p');collection.className='live-collection-progress';collection.textContent=ClipProgression.summary();deck.before(collection);
    this.ctx = this.canvas.getContext('2d');
    this.select = root.querySelector('[data-live-select]');
    clips.forEach((clip,index)=>{const option=document.createElement('option');option.value=index;option.textContent=`${clip.label} · N${clip.requiredLevel}`;this.select.append(option);});
    ...
    // One actual workspace shared by the apartment and the venue: clips above, monitors/controls below.
    const workspace=document.createElement('div');workspace.className='resolute-workspace';
    const consoleMain=document.createElement('section');consoleMain.className='resolute-console-main';consoleMain.setAttribute('aria-label','Clips, sorties et paramètres');
    const coach=document.createElement('aside');coach.className='resolute-console-coach';coach.setAttribute('aria-label','Conduite du show');
    for(const selector of ['.live-visual-suggestion','.live-collection-progress','.career-clip-deck','.live-desk'])consoleMain.append(root.querySelector(selector));
    for(const selector of ['.live-phase-track','.live-action-card','.live-cue','.live-response','.live-plan','.live-help'])coach.append(root.querySelector(selector));
    workspace.append(consoleMain,coach);root.append(workspace);
  }
  media(index) {
    const video=document.createElement('video');
    video.src=this.clips[index].src;video.muted=true;video.loop=true;video.playsInline=true;video.preload='auto';
    if(this.clips[index].nativeBpm)video.playbackRate=this.showProfile.bpm/this.clips[index].nativeBpm;
  ...
  start(index) {
    index=Math.max(0,Math.min(this.clips.length-1,Number(index)||0));
  ...
  prepare(index, byPlayer=false) {
    if(!Number.isInteger(index)||!this.clips[index])return false;
  ...
  suggestedEnergy() {
    const c=this.cues[Math.min(3,Math.floor(this.elapsed/15))],mid=(c.low+c.high)/2;
    return mid<40?'warmup':mid<60?'groove':mid<75?'rise':'peak';
  }
  updateCue() {
    ...
    let gesture=0,gestureLabel='Choisis une autre image dans la préview';
    if(index===0||index===3){gesture=Math.min(1,this.phaseStats[index].matched/5);gestureLabel=`${Math.min(5,this.phaseStats[index].matched).toFixed(1)} / 5 s dans la cible`;}
    else if(this.transition&&(index!==2||this.transition.synced)){gesture=.4+.6*Math.min(1,this.transition.elapsed/this.transition.duration);gestureLabel='Fondu en cours · laisse-le se terminer';}
    else if(this.queuedAt!=null){gesture=.35;gestureLabel=`Départ sur le temps 1 dans ${Math.max(0,this.queuedAt-this.elapsed).toFixed(1)} s`;}
    else if(this.preparedByPlayer){gesture=.25;gestureLabel=index===2?'Image prête · programme le fondu sur le temps 1':'Image prête · envoie ton fondu';}
    this.root.querySelector('[data-live-gesture]').value=done?1:gesture;
    this.root.querySelector('[data-live-gesture-label]').textContent=done?'Geste validé · maintiens l’ambiance':gestureLabel;
    this.root.querySelector('.live-action-card').dataset.success=String(done);
    const energyNames={warmup:'Calme / break',groove:'Groove',rise:'Montée',peak:'Peak'};
    this.root.querySelector('[data-live-visual-hint]').textContent=`Ambiance conseillée : ${energyNames[this.suggestedEnergy()]}. Tu gardes le choix de l’image.`;
    this.root.querySelector('[data-meter-band]').style.left=`${cue.low}%`;
    this.root.querySelector('[data-meter-band]').style.width=`${cue.high-cue.low}%`;
    this.root.querySelector('[data-meter-needle]').style.left=`${this.blackout?0:this.intensity*100}%`;
    ...
    this.phaseNodes.forEach((node,i)=>{const played=this.phaseStats[i].seconds>0;node.setAttribute('aria-current',i===index?'step':'false');node.dataset.done=String(this.stageTasks[i]);node.dataset.past=String(i<index&&played);node.querySelector('small').textContent=this.stageTasks[i]?'✓ Geste réussi':i<index?(played?'À retravailler':'Non joué'):i===index?`${Math.max(0,Math.ceil((i+1)*15-this.elapsed))} s restantes`:`${i*15}–${(i+1)*15} s`;});
