...
function createShopItems() {
  ...
  const addClip = (slug, label, cost, style, description, stats = {}) => {
    ...
    items.push(makeItem({
      ...
      stats: finalStats,
      styleTarget: style,
      visualTier:cost>=600?5:cost>=300?3:1,
      styleXp,
      apply(target) {
...
function normalizeSlot(data, index) {
  ...
  if(!merged.physical)merged.gigs = mergeStarterGigData(merged.gigs);
...
function renderDesktop() {
  ...
  if (currentApp === 'guide') window.PhysicalCareer ? PhysicalCareer.guide() : renderGuide();
  ...
  if (currentApp === 'stats') window.PhysicalV4?.enabled ? PhysicalCareer.stats() : renderStats();
  ...
  if (currentApp === 'skills') window.PhysicalV4?.enabled ? PhysicalCareer.skills() : renderSkills();
  if (currentApp === 'shop') renderShop();
  if (currentApp === 'files') window.VJFileExplorer?.render();
  addWindowControls();
}
...
function clampWindowBounds(width, height) {
  ...
  const maxHeight = Math.max(1, window.innerHeight - 132);
  return {
    width: Math.round(clamp(Number(width) || 1160, Math.min(360, maxWidth), maxWidth)),
    height: Math.round(clamp(Number(height) || 680, Math.min(300, maxHeight), maxHeight)),
  };
}
...
function getJobPay(target = profile) {
  ...
  return 40 + Math.min(20, Math.floor(days / 10) * 5);
}
...
function renderInventoryItem(item) {
  ...
  return `
    <article class="inventory-row" data-item-id="${escapeHtml(item.id)}">
      <div data-item-preview="${escapeHtml(item.id)}" class="product-preview mini product-${escapeHtml(item.type)} product-${escapeHtml(item.id)}" style="${getShopVisualStyle(item)}">
        <span></span>
...
function getResaleValue(item) {
  return Math.max(1, Math.round((Number(item?.cost) || 0) * 0.5));
}
...
function renderShop() {
  ...
  if (!activeCategory) {
    appWindow.innerHTML = `
      ...
      <section class="shop-marketing-hero">
        ...
        <div class="shop-showcase market-hero-photo" data-item-preview="projector-pro-6500"></div>
      </section>
      ...
      <div class="shop-portals premium-portals">
        ${Object.entries(categories).map(([category, data]) => `
          <button class="shop-portal shop-portal-${escapeHtml(data.icon)}" type="button" data-shop-view="${category}">
            <span class="shop-portal-art product-${escapeHtml(data.icon)}" data-item-preview="${category==='gear'?'computer-creator-laptop-rtx':category==='clothing'?'clothing-hoodie-vj':shopItems.find(i=>i.category==='vjloop').id}"></span>
  ...
  const sourceItems = shopItems.filter((item) => (item.category || 'gear') === shopView && !(shopView==='clothing'&&window.StudioWardrobe?.ownedMode&&!profile.ownedItems.includes(item.id)));
  const items = window.CatalogBrowser ? CatalogBrowser.filter(sourceItems) : sourceItems;
  appWindow.innerHTML = `
    ...
    ${window.CatalogBrowser?.toolbar(items)||''}
    ${shopView === 'clothing' ? `
    ...
    ` : `
      ...
      <div class="shop-hero-copy">
        <span class="market-eyebrow">${shopView==='vjloop'?'BIBLIOTHÈQUE / IMAGES EN MOUVEMENT':'LE MATÉRIEL DE TES PROCHAINS SHOWS'}</span>
        <h2>${shopView==='vjloop'?'Trouve la couleur de ton set.':'Du premier câble à la grande scène.'}</h2>
        <p>${shopView==='vjloop'?'Choisis tes collections selon la musique et ton niveau. Tes anciennes boucles restent disponibles.':'Inspecte les objets en 3D. Compare leur utilité, leur prix et ce que ton niveau débloque.'}</p>
      </div>
      <div class="market-hero-photo" data-item-preview="${shopView==='gear'?'projector-pro-6500':items[0]?.id||shopItems.find(i=>i.category==='vjloop').id}"></div>
    </div>`}
...
function renderGearShopSections(items) {
  const groups = [
    ...
    ['cable', 'Câbles vidéo, audio et alimentation'],
...
function renderShopDetail(item) {
  ...
  const physicalLock = window.PhysicalV4?.enabled ? PhysicalCareer.unlockItem(item) : '';
  ...
  appWindow.innerHTML = `
    ...
    <section class="product-detail-page">
      <div data-item-preview="${escapeHtml(item.id)}" class="product-detail-visual product-${escapeHtml(item.type)} product-${escapeHtml(item.id)}" style="${visualStyle}">
        <span></span>
        <i>${escapeHtml(getProductInitials(item.label))}</i>
      </div>
      <div class="product-detail-info">
        <span class="product-kicker">${item.category==='gear'?'MATÉRIEL DE SCÈNE':item.category==='clothing'?'TENUE / ACCESSOIRES':'COLLECTION VIDÉO'}</span>
        ...
        <div class="product-detail-price">
          ...
          <span>${owned ? (equipped ? 'Equipped' : `Owned${item.stackable ? ` x${count}` : ''}`) : lockedByLevel ? (physicalLock || `Niveau ${unlockLevel} requis`) : 'Disponible'}</span>
        </div>
        ...
        <div class="row-actions product-detail-actions">
          ...
          ${owned && item.category === 'clothing' ? `<button class="primary-action" type="button" data-wear="${item.id}">Porter</button>` : `<button class="primary-action" type="button" data-buy="${item.id}" ${canBuy ? '' : 'disabled'}>${lockedByLevel ? (physicalLock || `Niveau ${unlockLevel}`) : owned && !item.stackable ? 'Owned' : 'Acheter'}</button>`}
        </div>
      </div>
    </section>
  `;
...
function renderClothingStoreItem(item) {
  ...
  return `
    <article class="clothing-list-item" data-detail="${escapeHtml(item.id)}">
      <div data-item-preview="${escapeHtml(item.id)}" class="clothing-list-thumb product-${escapeHtml(item.type)} product-${escapeHtml(item.id)}" style="${visualStyle}">
        <span></span>
        <i>${escapeHtml(getProductInitials(item.label))}</i>
      </div>
...
function renderShopItem(item) {
  ...
  return `
    <article class="shop-card rarity-${escapeHtml(item.rarity || 'starter')}${item.styleTarget ? ` style-${escapeHtml(item.styleTarget)}` : ''}" data-detail="${escapeHtml(item.id)}">
      <div data-item-preview="${escapeHtml(item.id)}" class="product-preview product-${escapeHtml(item.type)} product-${escapeHtml(item.id)}" style="${visualStyle}">
        <span></span>
        <i>${escapeHtml(getProductInitials(item.label))}</i>
      </div>
      <div class="shop-card-body">
      ...
      <div class="shop-tags">
        <span>${item.physical ? (item.physical.unlock ? 'Après gig '+item.physical.unlock : 'Dès le départ') : 'Palier '+unlockLevel}</span>
        ${tagList.map((tag) => `<span>${escapeHtml(tag)}</span>`).join('')}
      </div>
      <details class="shop-card-specs"><summary>Avantages et compromis</summary><div class="shop-balance">
        <div>
      ...
      <div class="row-actions">
        <button class="secondary-action" data-open-detail="${item.id}">Fiche</button>
        <button class="primary-action" data-buy="${item.id}" ${lockedByOwned || lockedByLevel ? 'disabled' : ''}>${lockedByLevel ? (physicalLock || `Niveau ${unlockLevel}`) : lockedByOwned ? (equipped ? 'Equipped' : 'Owned') : item.stackable && owned ? 'Acheter +' : 'Acheter'}</button>
      </div>
    </article>
  `;
}
...
function getItemUnlockLevel(item) {
  ...
  if (item.category === 'vjloop') return item.visualTier||clamp(stars, 1, 8);
...
function addXp(amount) {
  ...
  while (s.xp >= s.level * 100) {
    ...
    const collection=ClipProgression.tiers.find(t=>t.level===s.level);
    const visualReward=collection?` Collection ${collection.label} débloquée dans ta régie : ${videoClips.filter(c=>ClipProgression.enrich(c).requiredLevel===s.level).length} clips supplémentaires. Tes anciens clips restent disponibles.`:'';
    addEmail('Carriere', `Niveau ${s.level}`, `Tu gagnes un niveau. Bonus de progression: ${bonus}. De nouveaux paliers Atazone peuvent se débloquer.${visualReward}`);
    notify(`Niveau ${s.level} atteint : ${bonus}.${collection?` Nouvelle collection : ${collection.label} !`:''}`);
  }
}
...
function setupScene() {
  ...
  floor.userData.placementSurface = 'Sol';
...
function createDeskStation() {
  ...
  top.position.y = 0.82;
  top.userData.placementSurface = 'Bureau';
  top.castShadow = true;
...
function selectVideo(index) {
  const clip=videoClips[index];if(!clip)return;
  if(!ClipProgression.unlocked(clip)){notify(`Ce clip se débloque au niveau ${clip.requiredLevel}.`);return;}
  if (liveShow) { const local=liveShow.clips.findIndex(c=>c.src===clip.src);if(local>=0)liveShow.prepare(local,true);return; }
...
function animate() {
  ...
  window.StudioDecor?.tick();
  if(!window.PhysicalV4?.enabled)window.FirstShowCoach?.tick(liveDelta);
  window.ClubAtmosphere?.tick();
  window.VenueDesign?.tick();
  window.PhysicalV4?.tick(liveDelta);
...
function calculateProjectionCorners(rig) {
  ...
  const opticalScale=currentGig?.physical&&rig.physicalId?PhysicalCore.opticalScale(PhysicalV4.state,rig.physicalId):1;
  const baseW = rig.screen.width * distanceScale * opticalScale;
  const baseH = rig.screen.height * distanceScale * opticalScale;
...
function updateProjectionMeshes(rig) {
  ...
  rig.screenProjection.material.opacity = (ready ? 0.96 : 0.62 + rig.calibration.coverage / 300) * (currentGig?.physical && rig.physicalId ? Math.min(1, PhysicalCore.spec(PhysicalCore.get(PhysicalV4.state, rig.physicalId)?.modelId).lumens / 5000) : GearCapabilities.brightness(GearCapabilities.selected('projector')));
  rig.wallProjection.material.opacity = rig.cable.connected ? 0.22 : 0.09;
}

function applyProjectionTexture(rig) {
  const texture = currentGig?.physical ? PhysicalOutputs.texture(PhysicalV4.state,rig,currentGig) : rig.cable.connected ? (liveTexture || clipTexture) : testCardTexture;
...
function addMaskPointFromHit(hit) {
  ...
  if(currentGig?.physical&&rig.referenceMarkers){const target=rig.mappingTargets?.[rig.mappingZone||0]?.find(p=>Math.hypot(p.x-local.x,p.y-local.y)<.08);if(target){local.x=target.x;local.y=target.y;}}
...
function startLivePerformance({restoring=false}={}) {
  ...
  liveShow = new VJLiveShow(root, videoClips,{saved:restoring?profile.activeRun?.live:null});
  liveShow.start(0);
  ...
  resultContent.innerHTML = `
    <div class="result-list">
      <div class="contract-debrief"><section class="client-reaction"><small>TON CLIENT</small><p>${escapeHtml(clientFeedback)}</p>${ClientRelations.markup(currentGig)}</section>
        ...
        ${liveResult.stageTasks ? `<p class="contract-practiced">Défis de scène : ${liveResult.stageTasks.filter(Boolean).length}/4 — ${['ambiance','nouvelle image','fondu sur le rythme','finale'].map((name,i)=>`${liveResult.stageTasks[i]?'✓':'à pratiquer :'} ${name}`).join(' · ')}.</p>` : ''}
