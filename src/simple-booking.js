...
function renderSimpleBookings() {
  ...
  const goal = g => g.physical ? g.requirement : getGigMaskRequirement(g) > 0
  ...
  const card = (g, current = false) => {
    ...
    if (g.status === 'open') {
      action = blockers.length
        ...
        : `<button class="primary-action" data-simple-apply="${g.id}">Choisir ce contrat →</button>`;
    ...
    return `<article class="simple-gig ${current ? 'recommended' : ''}">
      ...
      <h2>${escapeHtml(g.title)}</h2>${ClientRelations.markup(g)}${g.referredBy?`<p>Recommandé par ${escapeHtml(g.referredBy)}</p>`:''}
      ...
      ${action}
      <details>
        <summary>Brief et prérequis</summary><p>${escapeHtml(ShowProfiles.brief(g))}</p>
