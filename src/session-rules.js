...
window.SessionRules={
 ...
 simplifySetup(gig){
  ...
  const brief=document.createElement('section');brief.className='session-brief';brief.innerHTML=`<small>TON OBJECTIF</small><h2>${getGigProjectorCount(gig)>1?'Coordonne tes projections':'Crée un show lisible'}</h2><p>${getGigMaskRequirement(gig)>0?'Délimite la forme avec un masque.':'Connecte et cadre la projection.'} Accompagne les passages musicaux et termine un fondu.</p><p>${escapeHtml(ShowProfiles.brief(gig))}</p><p>Ton matériel compatible est présélectionné. Vérifie les frais ci-dessous, puis enregistre ta préparation.</p><small>La note dépend de l’installation et du cadrage (45 %), de l’intensité (45 %) et d’un fondu terminé (10 %). Les gestes réellement réussis donnent de l’expérience terrain.</small>`;gigSetupContent.prepend(brief);if(gigSetupStartButton.disabled)details.open=true;
