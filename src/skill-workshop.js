...
(()=>{
  ...
  window.renderSkills=function renderSkills(){
    ...
    appWindow.innerHTML=`<div class="app-heading"><div><h1>VJ School & Skills</h1><p>Apprends, pratique, puis prouve tes compétences sur le terrain. Les niveaux ouvrent maintenant des capacités visibles dans la régie, en plus des contrats et du matériel.</p></div></div><div class="shop-grid skills-grid">${cards}</div>`;
  ...
  window.addEventListener('vj-progression',()=>{if(typeof currentApp!=='undefined'&&currentApp==='skills'){window.renderSkills();addWindowControls();}});
})();
